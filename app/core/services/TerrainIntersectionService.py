...
class SRTMProvider(DEMProvider):
	...
	def __init__(self):
		...
		try:
			...
			# Prefer cached data; allow automatic download when first queried
			self._elevation_data.add_elevation_data()
		except Exception:
			self._elevation_data = None
	...
	def get_elevation(self, latitude: float, longitude: float) -> Optional[float]:
		if self._elevation_data is None:
			return None
		try:
			val = self._elevation_data.get_elevation(latitude, longitude)
